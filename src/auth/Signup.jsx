...
export default function Signup({
  ...
  async function signUp(data) {
    try {
      ...
      setMessage(
        "Password must have one uppercase, one numeric and be eight length longs characters and/or invalid email adress format."
      );
      console.log(error);
      setLoading(false);
    }
  ...
  return (
    <>
      <StyledFormCont as={animated.div} style={fade}>
        ...
        {message !== undefined ? (
          <StyledSpanMessage style={{ marginBottom: "8%" }}>
            {t(`${message}`)}
          </StyledSpanMessage>
        ) : (
          <></>
        ...
        >
          ...
          <StyledInputContainer>
            <StyledInputForm
              {...register("password")}
              type={!revealedone ? "password" : "text"}
              required
            />
            <StyledImgPassword
              src={!revealedone ? eyepasswordclose : eyepassword}
              onClick={() => {
                setRevealedone(!revealedone);
              }}
            />
          ...
          <StyledSpan>
            {t("signup.account")}
            <BtnLinkLog type="button" onClick={() => navigateTo("login")}>
              {t("signup.here")}
            </BtnLinkLog>
          </StyledSpan>
          <StyledSpan>
